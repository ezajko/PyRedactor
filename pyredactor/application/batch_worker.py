...
class BatchOperationWorker(QObject):
    ...
    def execute_batch_operation(self):
        ...
        try:
            self.progress_update.emit(f"Initializing {self.operation_name}...", 0)

            # Execute the batch operation
            if callable(self.update_func):
                self.progress_update.emit(f"Executing {self.operation_name}...", 50)
                ...
                error_msg = "Invalid update function provided - batch operation cannot be executed"
